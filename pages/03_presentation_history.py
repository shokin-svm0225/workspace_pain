...
#データの加工方法の指定
options = ['2024年11月8日', '2024年11月22日', '2024年12月13日', '2025年1月24日', '2025年4月25日', '2025年5月23日', '2025年6月13日', '2025年7月11日', '2025年10月17日', '2025年11月7日', '2025年11月28日', '2025年12月5日', '2025年12月19日', '2026年1月16日', '2026年2月17日']
...
if data_processing == "2024年11月8日":
    ...
    st.subheader('学術講演会の事前準備', divider='rainbow')
    st.pdf("picture/学術講演会原稿_完成_岩崎翔太.pdf", height=600)

elif data_processing == "2025年12月5日":
    st.subheader('学術講演会の反省会', divider='rainbow')
    st.markdown("""
    **質問**
    - 正答率はどのように出している？ by利根川先生
    - 山登り法にランダムジャンプを入れているが、一番良いスコアの時からスコアの停滞で低いスコアにジャンプした場合の対策は？ by西川先生 \n
    **反省**
    - 聞いている側が何もわからない前提で質疑応答の対策を考える by水野先生 \n
    **今後の予定**
    - 主成分分析＋山登り法＋ランダムジャンプによる実験として、ガウスカーネルおよびシグモイドカーネルにおける質問項目の重み付け
    - 主成分分析＋山登り法＋ランダムジャンプによる実験として、SVMのハイパーパラメータ最適化
    - 進化計算での山登り法
    - 教師なし学習で試すのはあり、som（クラスタリング）
      - yちかくにあるデータ同士を近づけていく
      - 重み付けや主成分分析の導入もできる
    - 決定木やるならlightgbm
    - 主成分分析で3次元に落とし込んでからsom
    """)

elif data_processing == "2025年12月19日":
    with st.container(border=True):
        st.subheader("目次", divider='rainbow')
        st.markdown("""
        - 実験1
...
elif data_processing == "2026年1月16日":
    st.title('本日の発表内容')
    ...
    with st.container(border=True):
        ...
        **実験結果**\n
        ...
        - :red[標準化なし]
        - ガウスカーネル（gamma）\n
        PainDITECTは、ユーグリッドの距離のおよそ平均で60とすると、1/e（0.37）を基準で、[0.016]:red[実験：0.01,0.016,0.05][実行時間：2191秒]\n
        BS-POPは、ユーグリッドの距離のおよそ平均で10とすると、1/e（0.37）を基準で、[0.1]:red[実験：0.01,0.1,0.2][実行時間：3310秒]\n
        FUSIONは、ユーグリッドの距離のおよそ平均で70とすると、1/e（0.37）を基準で、[0.014]:red[実験：0.01,0.014,0.03][実行時間：6869秒]\n
        - シグモイドカーネル（gamma,coef0）\n
        PainDITECTは、内積のおよそ平均で130とすると、tanh（1,-1）を基準で、[gamma：-0.008〜0.008]:red[実験：0.001,0.004,0.008]、gammaで抑えられるので[coef0：0][実行時間：1517秒]\n
        BS-POPは、内積のおよそ平均で40とすると、tanh（1,-1）を基準で、[gamma：-0.025〜0.025]:red[実験：0.01,0.02,0.025]、gammaで抑えられるので[coef0：0][実行時間：2278秒]\n
        FUSIONは、内積のおよそ平均で170とすると、tanh（1,-1）を基準で、[gamma：-0.006〜0.006]:red[実験：0.001,0.003,0.006]、gammaで抑えられるので[coef0：0][実行時間：4732秒]\n
        - 多項式カーネル（gamma,coef0,degree）\n
        PainDITECTは、内積のおよそ平均で130とすると、[gamma：0.008付近]:red[実験：0.003,0.008,0.01]、gammaで抑えられるので[coef0：0]、大きく変化するのを防ぐため[degree：2,3][実行時間：2813秒]\n
        BS-POPは、内積のおよそ平均で40とすると、[gamma：0.025付近]:red[実験：0.01,0.025,0.04]、gammaで抑えられるので[coef0：0]、大きく変化するのを防ぐため[degree：2,3][実行時間：3479秒]\n
        FUSIONは、内積のおよそ平均で170とすると、[gamma：0.006付近]:red[実験：0.001,0.006,0.01]、gammaで抑えられるので[coef0：0]、大きく変化するのを防ぐため[degree：2,3][実行時間：7976秒]\n
                    
        - :red[標準化あり]
        - ガウスカーネル（gamma）\n
        PainDITECTは、ユーグリッドの距離のおよそ平均で25とすると、1/e（0.37）を基準で、[0.04]:red[実験：0.01,0.04,0.1][実行時間：2159秒]\n
        BS-POPは、ユーグリッドの距離のおよそ平均で37とすると、1/e（0.37）を基準で、[0.027]:red[実験：0.01,0.027,0.1][実行時間：3284秒]\n
        FUSIONは、ユーグリッドの距離のおよそ平均で63とすると、1/e（0.37）を基準で、[0.016]:red[実験：0.01,0.016,0.04][実行時間：6983秒]\n
        - シグモイドカーネル（gamma,coef0）\n
        PainDITECTは、内積のおよそ平均で0.18とすると、tanh（1,-1）を基準で、[gamma：-5.555〜5.555]:red[実験：1,3,5.5]、gammaで抑えられるので[coef0：0][実行時間：1782秒]\n
        BS-POPは、内積のおよそ平均で0.002とすると、tanh（1,-1）を基準で、[gamma：-500〜500]:red[実験：50,250,500]、gammaで抑えられるので[coef0：0][実行時間：1747秒]\n
        FUSIONは、内積のおよそ平均で0.18とすると、tanh（1,-1）を基準で、[gamma：-5.555〜5.555]:red[実験：1,3,5.5]、gammaで抑えられるので[coef0：0][実行時間：4545秒]\n
        - 多項式カーネル（gamma,coef0,degree）\n
        PainDITECTは、内積のおよそ平均で0.18とすると、[gamma：5.555付近]:red[実験：5,5.5,6]、gammaで抑えられるので[coef0：0]、大きく変化するのを防ぐため[degree：2,3][実行時間：4047秒]\n
        BS-POPは、内積のおよそ平均で0.002とすると、[gamma：500付近]:red[実験：400,500,600]、gammaで抑えられるので[coef0：0]、大きく変化するのを防ぐため[degree：2,3][実行時間：秒]\n
        FUSIONは、内積のおよそ平均で0.18とすると、[gamma：5.555付近]:red[実験：5,5.5,6]、gammaで抑えられるので[coef0：0]、大きく変化するのを防ぐため[degree：2,3][実行時間：秒]\n
        """)
        # タブの作成
    ...
    with st.container(border=True):
        ...
        - 修士論文題名はどうするか？
        - 修論のタイトル
        - SVMを用いた整形外科における疼痛原因診断の性能評価
        - Evaluating the Performance of SVM for Diagnosing Causes of Pain in Orthopedics 
        """)
...
elif data_processing == "2026年2月17日":
    st.subheader('修論期限間近ですよ、執筆してください', divider='rainbow')
