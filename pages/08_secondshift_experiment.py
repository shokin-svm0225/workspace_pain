...
with col1:
    with st.container(border=True):
        st.subheader('山登り法（ローカルベスト）', divider='rainbow')
        st.markdown("""
        現在までのベストスコアを基準にせず、
...
if st.button("開始", help="実験の実行"):
    ...
    # Cのグリッドサーチ（外側ループ）
    for C in C_values:
        weights_change, score, X_val_tmp, y_val_tmp, pred_tmp, score_history = hill_climbing(datas, labels, C, initial_weights, max_iter_1=100, step_size=0.1)
